...
#define MT_CANS_LEN         (9)          // 控制域C 地址域A AFN SEQ字节总长 huyuxiang add version leng
...
typedef enum
{
    ...
    CMD_AFN_4_F15_HAVE_DIFF_EVENT    = 0x040F,       //继电器输出控制方案
...
typedef struct
{
    UINT8  ulMainVersion;    // 主用IP版本信息
    UINT32 ulMainIP;        // 主用地址
    UINT16 usMainPort;      // 主用端口
    UINT8  ulBackVersion;    // 备用IP版本信息
    UINT32 ulBackIP;        // 备用地址
    UINT16 usBackPort;      // 备用端口 
    UINT8  ucascAPN[16];    // APN
    UINT8  virUserName[32]; //虚拟专网用户名
    UINT8  virUserPw[32];   //虚拟专网密码
...
typedef struct
{
    sMtFmt19_f sBegin;        // 开始继电器时间端
    sMtFmt19_f sEnd;          // 关闭继电器时间端
